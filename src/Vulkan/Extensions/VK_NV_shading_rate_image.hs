...
--     of @shadingRatePaletteEntryCount@ valid 'ShadingRatePaletteEntryNV'
--     values
--
...
--     effective shading rate for fragment processing. It also indicates
--     that the implementation supports the @ShadingRateNV@ SPIR-V
--     execution mode.
--
-- -   #features-shadingRateCoarseSampleOrder#
--     @shadingRateCoarseSampleOrder@ indicates that the implementation
--     supports a user-configurable ordering of coverage samples in
...
--     structure.
--
-- -   #limits-shading-rate-texel-size# @shadingRateTexelSize@ indicates
--     the width and height of the portion of the framebuffer corresponding
--     to each texel in the shading rate image.
--
-- -   #limits-shading-rate-palette-size# @shadingRatePaletteSize@
...
--     coverage samples supported in a single fragment. If the product of
--     the fragment size derived from the base shading rate and the number
--     of coverage samples per pixel exceeds this limit, the final shading
--     rate will be adjusted so that its product does not exceed the limit.
--
