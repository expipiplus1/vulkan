...
-- = Description
--
-- -   #features-transformFeedback# @transformFeedback@ indicates whether
--     the implementation supports transform feedback and shader modules
--     /can/ declare the @TransformFeedback@ capability.
--
...
--     structure.
--
-- -   #limits-maxTransformFeedbackStreams# @maxTransformFeedbackStreams@
--     is the maximum number of vertex streams that can be output from
--     geometry shaders declared with the @GeometryStreams@ capability. If
...
--     is the maximum number of transform feedback buffers that can be
--     bound for capturing shader outputs from the last vertex processing
--     stage.
--
...
--     in bytes for each vertex that captured to one or more transform
...
--     in bytes for each vertex that can be captured to a specific
...
--
-- -   #limits-transformFeedbackQueries# @transformFeedbackQueries@ is true
--     if the implementation supports the
--     'Vulkan.Core10.Enums.QueryType.QUERY_TYPE_TRANSFORM_FEEDBACK_STREAM_EXT'
--     query type. @transformFeedbackQueries@ is false if queries of this
...
--     @transformFeedbackStreamsLinesTriangles@ is false the implementation
--     only supports an @OpExecutionMode@ of @OutputPoints@ when more than
--     one vertex stream is output from the geometry shader.
--
...
--
-- -   #limits-transformFeedbackDraw# @transformFeedbackDraw@ is true if
--     the implementation supports the 'cmdDrawIndirectByteCountEXT'
