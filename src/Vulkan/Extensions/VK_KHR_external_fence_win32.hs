...
--
-- -   #VUID-vkGetFenceWin32HandleKHR-device-parameter# @device@ /must/ be
...
--
-- -   #VUID-vkGetFenceWin32HandleKHR-pHandle-parameter# @pHandle@ /must/
...
-- == Valid Usage
--
-- -   #VUID-vkImportFenceWin32HandleKHR-fence-04448# @fence@ /must/ not be
--     associated with any queue command that has not yet completed
--     execution on that queue
--
-- == Valid Usage (Implicit)
--
-- -   #VUID-vkImportFenceWin32HandleKHR-device-parameter# @device@ /must/
