...
--
-- -   #VUID-vkGetSemaphoreFdKHR-device-parameter# @device@ /must/ be a
--     valid 'Vulkan.Core10.Handles.Device' handle
--
-- -   #VUID-vkGetSemaphoreFdKHR-pGetFdInfo-parameter# @pGetFdInfo@ /must/
--     be a valid pointer to a valid 'SemaphoreGetFdInfoKHR' structure
--
-- -   #VUID-vkGetSemaphoreFdKHR-pFd-parameter# @pFd@ /must/ be a valid
...
-- == Valid Usage
--
-- -   #VUID-vkImportSemaphoreFdKHR-semaphore-01142# @semaphore@ /must/ not
--     be associated with any queue command that has not yet completed
