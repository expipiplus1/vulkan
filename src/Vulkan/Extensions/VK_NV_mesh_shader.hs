...
-- == Valid Usage (Implicit)
--
-- -   #VUID-VkPhysicalDeviceMeshShaderFeaturesNV-sType-sType# @sType@
--     /must/ be
...
--     that /can/ be launched by a single draw mesh tasks command. See
...
--     shader invocations in a single local workgroup. The product of the
--     X, Y, and Z sizes, as specified by the @LocalSize@ execution mode in
--     shader modules or by the object decorated by the @WorkgroupSize@
...
--     @z@ sizes, as specified by the @LocalSize@ execution mode or by the
...
--     memory size used by the mesh shader, which must be less than or
--     equal to @maxMeshTotalMemorySize@.
--
-- -   @meshOutputPerPrimitiveGranularity@ is the granularity with which
--     mesh outputs qualified as per-primitive are allocated. The value can
--     be used to compute the memory size used by the mesh shader, which
...
-- == Valid Usage (Implicit)
--
-- -   #VUID-VkPhysicalDeviceMeshShaderPropertiesNV-sType-sType# @sType@
--     /must/ be
