...
instance ToCStruct CudaLaunchInfoNV where
  ...
  pokeCStruct p CudaLaunchInfoNV{..} f = evalContT $ do
    ...
    pParams'' <- if Data.Vector.null (params)
      then pure nullPtr
      else do
        ...
        pure $ pPParams
    lift $ poke ((p `plusPtr` 64 :: Ptr (Ptr (Ptr ())))) pParams''
    ...
    pExtras'' <- if Data.Vector.null (extras)
      then pure nullPtr
      else do
        ...
        pure $ pPExtras
    lift $ poke ((p `plusPtr` 80 :: Ptr (Ptr (Ptr ())))) pExtras''
    lift $ f
  cStructSize = 88
  cStructAlignment = 8
  pokeZeroCStruct p f = do
...
instance FromCStruct CudaLaunchInfoNV where
  peekCStruct p = do
    ...
    let pParamsLength = if pParams == nullPtr then 0 else (fromIntegral paramCount')
    pParams' <- generateM pParamsLength (\i -> peek @(Ptr ()) ((pParams `advancePtrBytes` (8 * (i)) :: Ptr (Ptr ()))))
    ...
    let pExtrasLength = if pExtras == nullPtr then 0 else (fromIntegral extraCount')
    pExtras' <- generateM pExtrasLength (\i -> peek @(Ptr ()) ((pExtras `advancePtrBytes` (8 * (i)) :: Ptr (Ptr ()))))
