...
instance Show DescriptorBindingFlagBits where
  showsPrec p e = case lookup e showTableDescriptorBindingFlagBits of
    Just s -> showString enumPrefixDescriptorBindingFlagBits . showString s
    Nothing ->
      let DescriptorBindingFlagBits x = e
      in  showParen (p >= 11) (showString conNameDescriptorBindingFlagBits . showString " 0x" . showHex x)
...
instance Read DescriptorBindingFlagBits where
  readPrec = parens
    (   Text.ParserCombinators.ReadPrec.lift
        (do
          skipSpaces
          _ <- string enumPrefixDescriptorBindingFlagBits
          asum ((\(e, s) -> e <$ string s) <$> showTableDescriptorBindingFlagBits)
        )
    +++ prec
          10
          (do
            expectP (Ident conNameDescriptorBindingFlagBits)
            v <- step readPrec
