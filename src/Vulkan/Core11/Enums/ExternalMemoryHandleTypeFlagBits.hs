...
instance Show ExternalMemoryHandleTypeFlagBits where
  showsPrec p e = case lookup e showTableExternalMemoryHandleTypeFlagBits of
    Just s -> showString enumPrefixExternalMemoryHandleTypeFlagBits . showString s
    Nothing ->
      let ExternalMemoryHandleTypeFlagBits x = e
      in  showParen (p >= 11) (showString conNameExternalMemoryHandleTypeFlagBits . showString " 0x" . showHex x)
...
instance Read ExternalMemoryHandleTypeFlagBits where
  readPrec = parens
    (   Text.ParserCombinators.ReadPrec.lift
        (do
          skipSpaces
          _ <- string enumPrefixExternalMemoryHandleTypeFlagBits
          asum ((\(e, s) -> e <$ string s) <$> showTableExternalMemoryHandleTypeFlagBits)
        )
    +++ prec
          10
          (do
            expectP (Ident conNameExternalMemoryHandleTypeFlagBits)
            v <- step readPrec
