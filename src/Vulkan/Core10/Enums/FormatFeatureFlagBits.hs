...
instance Show FormatFeatureFlagBits where
  showsPrec p e = case lookup e showTableFormatFeatureFlagBits of
    Just s -> showString enumPrefixFormatFeatureFlagBits . showString s
    Nothing ->
      let FormatFeatureFlagBits x = e
      in  showParen (p >= 11) (showString conNameFormatFeatureFlagBits . showString " 0x" . showHex x)
...
instance Read FormatFeatureFlagBits where
  readPrec = parens
    (   Text.ParserCombinators.ReadPrec.lift
        (do
          skipSpaces
          _ <- string enumPrefixFormatFeatureFlagBits
          asum ((\(e, s) -> e <$ string s) <$> showTableFormatFeatureFlagBits)
        )
    +++ prec
          10
          (do
            expectP (Ident conNameFormatFeatureFlagBits)
            v <- step readPrec
