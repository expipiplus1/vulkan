...
instance Show PipelineStageFlagBits where
  showsPrec p e = case lookup e showTablePipelineStageFlagBits of
    Just s -> showString enumPrefixPipelineStageFlagBits . showString s
    Nothing ->
      let PipelineStageFlagBits x = e
      in  showParen (p >= 11) (showString conNamePipelineStageFlagBits . showString " 0x" . showHex x)
...
instance Read PipelineStageFlagBits where
  readPrec = parens
    (   Text.ParserCombinators.ReadPrec.lift
        (do
          skipSpaces
          _ <- string enumPrefixPipelineStageFlagBits
          asum ((\(e, s) -> e <$ string s) <$> showTablePipelineStageFlagBits)
        )
    +++ prec
          10
          (do
            expectP (Ident conNamePipelineStageFlagBits)
            v <- step readPrec
