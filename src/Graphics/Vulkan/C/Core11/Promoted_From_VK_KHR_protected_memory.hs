...
data VkDeviceQueueInfo2 = VkDeviceQueueInfo2
  ...
  vkPNext :: Ptr ()
  , -- | @flags@ is a 'Graphics.Vulkan.C.Core10.Device.VkDeviceQueueCreateFlags'
...
data VkPhysicalDeviceProtectedMemoryProperties = VkPhysicalDeviceProtectedMemoryProperties
  ...
  -- <https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html#memory-protected-access-rules protected memory access rules>
  -- are broken. If @protectedNoFault@ is
  -- 'Graphics.Vulkan.C.Core10.Core.VK_TRUE', breaking those rules will not
...
--     does not include a 'VkProtectedSubmitInfo' structure, then each
--     element of the command buffer of the @pCommandBuffers@ array /must/
...
data VkProtectedSubmitInfo = VkProtectedSubmitInfo
  ...
  -- @protectedSubmit@ is 'Graphics.Vulkan.C.Core10.Core.VK_TRUE', the batch
  -- is protected. If @protectedSubmit@ is
  -- 'Graphics.Vulkan.C.Core10.Core.VK_FALSE', the batch is unprotected. If
  -- the 'Graphics.Vulkan.C.Core10.Queue.VkSubmitInfo'::@pNext@ chain does
