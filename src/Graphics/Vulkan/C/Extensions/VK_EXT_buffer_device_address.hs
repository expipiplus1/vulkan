...
instance Zero VkBufferDeviceAddressCreateInfoEXT where
  zero = VkBufferDeviceAddressCreateInfoEXT VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_CREATE_INFO_EXT
                                            zero
                                            zero
...
instance Zero VkBufferDeviceAddressInfoEXT where
  zero = VkBufferDeviceAddressInfoEXT VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO_EXT
                                      zero
                                      zero
...
instance Zero VkPhysicalDeviceBufferDeviceAddressFeaturesEXT where
  zero = VkPhysicalDeviceBufferDeviceAddressFeaturesEXT VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_BUFFER_DEVICE_ADDRESS_FEATURES_EXT
                                                        zero
                                                        zero
                                                        zero
                                                        zero
...
#else
vkGetBufferDeviceAddressEXT :: DeviceCmds -> ("device" ::: VkDevice) -> ("pInfo" ::: Ptr VkBufferDeviceAddressInfoEXT) -> IO VkDeviceAddress
vkGetBufferDeviceAddressEXT deviceCmds = mkVkGetBufferDeviceAddressEXT (pVkGetBufferDeviceAddressEXT deviceCmds)
foreign import ccall
#if !defined(SAFE_FOREIGN_CALLS)
  unsafe
#endif
