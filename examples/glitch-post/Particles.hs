...
into a third, static SSBO of target positions ('allocateParticleTargets');
...
module Particles
  ( ParticlePipeline (..)
  , allocateParticlePipeline
  , allocateParticleBuffers
  , allocateParticleTargets
  , allocateParticleSets
  , dispatchParticleSim
  , drawParticles
  ) where
...
allocateParticlePipeline dev colorFormat depthFormat = do
  -- The graphics half.
  ...
  (_, pipeline) <-
    Dynamic.allocatePipelineFromShaders
      ...
      zero
        ...
        , Dynamic.layout = Just pipelineLayout -- set 0: camera UBO + state SSBO
        }
...
-}
allocateParticleBuffers :: VMA.Allocator -> ResourceT IO (V.Vector Vk.Buffer)
allocateParticleBuffers vma =
  V.replicateM 2 $
    snd
      <$> Buffer.deviceBuffer
        vma
        (fromIntegral (particleCount * bytesPerParticle))
...
needed.
-}
allocateParticleTargets :: VMA.Allocator -> ResourceT IO Vk.Buffer
allocateParticleTargets vma = do
...
targetFloats =
  ...
  where
    cells = messageCells
    -- Sized to prevent text getting clipped by spiral's quads
    cellSize = 0.056
...
allocateParticleSets
  ...
  -> ParticlePipeline
  -> V.Vector Vk.Buffer
  -- ^ The two state buffers ('allocateParticleBuffers').
  -> Vk.Buffer
  -- ^ The glyph-pixel targets ('allocateParticleTargets').
  -> V.Vector Vk.Buffer
...
allocateParticleSets dev pp stateBuffers targetBuffer cameraBuffers = do
  (_, pool) <-
    ...
    void main() {
      ...
      if (p.posAge.w <= 0.0) {
        ...
        if (phase > 0.0 && goal.z > 0.5) {
          ...
          // Tuned to smooth transitions between text and spark forms.
          p.posAge.w = max(p.posAge.w, 1.0 + 2.5 * hash(float(i) * 4.7));
        }
