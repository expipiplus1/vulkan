...
import Glitch (GlitchPipeline (..), allocateGlitchDescriptorSets, allocateGlitchPipeline, dispatchGlitch)
import qualified Music.Player as Player
import Music.Tune (withTune)
import Particles (ParticlePipeline, allocateParticleBuffers, allocateParticlePipeline, allocateParticleSets, allocateParticleTargets, dispatchParticleSim, drawParticles)
import RenderTarget (allocateTarget, createDepthTarget)
import qualified Sound.OpenAL.FFI.AL as AL
import Tune (Knobs (..), eventsBetween, knobsAt)
import Tunnel (CameraSlot (..), TunnelPipeline (..), allocateCameraSlots, allocateTunnelGeometrySet, allocateTunnelPipeline, allocateTunnelVertexBuffer, dispatchTunnelGeometry, tunnelProjection, tunnelTwistRate, tunnelVertexCount)
...
main = runResourceT $ do
      ...
      WindowedConfig
        ...
        , swapchainConfig =
            defaultSwapchainConfig
              ...
              , -- The post pass writes display-referred colours into a UNORM
                -- image that is blitted as-is; an sRGB pick would re-encode.
  ...
  tunnelPL <- allocateTunnelPipeline dev sceneFormat depthFormat
  vertexBuffer <- allocateTunnelVertexBuffer vma
  cameraSlots <- allocateCameraSlots vma dev (tpDescriptorSetLayout tunnelPL) framesInFlight vertexBuffer
  geometrySet <- allocateTunnelGeometrySet dev (tpGeometrySetLayout tunnelPL) vertexBuffer
  particlePL <- allocateParticlePipeline dev sceneFormat depthFormat
  particleBuffers <- allocateParticleBuffers vma
  particleTargets <- allocateParticleTargets vma
  (particleSimSets, particleDrawSets) <-
    allocateParticleSets dev particlePL particleBuffers particleTargets (csBuffer <$> cameraSlots)
  glitchPL <- allocateGlitchPipeline dev
  ...
  -- (see 'dRendered'), which calls song zero.
  _alContext <- snd <$> allocate Player.open Player.close
  ...
  -- Seeded just below zero so the t=0 Intro boundary traces on the first
  -- frame ('eventsBetween' excludes its lower bound).
  songPos <- liftIO $ newIORef (-1)
  twist <- liftIO $ newIORef 0
  rendered <- liftIO $ newIORef 0
  let demo =
        Demo
          { dTunnel = tunnelPL
          , dGlitch = glitchPL
  ...
  runWindowLoop
    ...
    WindowLoop
      { wlMkState = allocateTargets vma dev glitchPL
...
data Demo = Demo
  ...
  -}
  , dRendered :: IORef Int
  {- ^ Frames whose commands reached the queue. A swapchain error aborts
  a frame mid-record but 'fIndex' still advances, so the first-frame
  one-shots and the parity indexing key on this count instead.
  -}
  }
...
allocateTargets vma dev glitchPL sc = do
...
drawFrame vc Demo{..} targets f = do
  ...
  rendered <- liftIO $ readIORef dRendered
  ...
  let
    ...
    -- Keyed by submitted-frame parity ('dRendered', not 'fIndex', so an
    -- aborted frame doesn't flip it under the ping-pong): the frame fences
    -- retire submissions in queue order, so the slot's previous user is
    -- done before this record overwrites it (see 'allocateCameraSlots').
    parity = rendered `mod` framesInFlight
  ...
  commands <- recordCommands vc f \cb -> do
    ...
    Blit.blitImage sExtent postImage swapImage cb
    swapchainToPresent cb swapImage

  queueSubmitFrame vc f imageIndex [commands]
  -- Submitted: the GPU runs this frame even if the present below throws.
  liftIO $ modifyIORef' dRendered (+ 1)
