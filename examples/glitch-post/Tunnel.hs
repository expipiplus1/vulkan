...
module Tunnel
  ( TunnelPipeline (..)
  , allocateTunnelPipeline
  , CameraSlot (..)
  , allocateCameraSlots
  , allocateTunnelVertexBuffer
  , allocateTunnelGeometrySet
...
'Dynamic.allocatePipelineFromShaders', which otherwise would default to an
...
allocateTunnelPipeline dev colorFormat depthFormat = do
  -- The graphics half.
  ...
  (_, pipeline) <-
    Dynamic.allocatePipelineFromShaders
      ...
      zero
        ...
        , Dynamic.layout = Just pipelineLayout -- set 0: camera UBO + vertex SSBO
        }
...
-}
allocateTunnelVertexBuffer :: VMA.Allocator -> ResourceT IO Vk.Buffer
allocateTunnelVertexBuffer vma = do
  (_, (buffer, _, _)) <-
    VMA.withBuffer
      vma
      zero
...
allocateTunnelGeometrySet dev layout buffer = do
  (_, pool) <-
...
allocateCameraSlots vma dev layout n vertexBuffer = do
