...
@buffer_reference@ needs SPIR-V 1.3+, so this is compiled with
@--target-env vulkan1.2@ ('compileShaderQ' rather than the plain @comp@
quasiquoter). The whole interface is reflected in "Pathtracer".
-}
...
code =
  $( compileShaderQ
       ...
       [glsl|
        ...
        // All members are vec3, so they share std140 base alignment 16 (the gl-block
        // layout guardrail wants non-increasing alignments).
        ...
        struct Sphere {
          ...
          vec4 albedo;       // rgb: albedo
          vec4 material;     // x: type (0 lambertian, 1 metal, 2 dielectric), y: fuzz, z: ior
        };
        ...
        // A BVH node reached by device address. Internal nodes have child addresses and
        // sphereIndex < 0; leaves have sphereIndex >= 0 (into the Scene buffer) and null
        // children. Fields are ordered by non-increasing alignment (16,16,8,8,4).
        ...
        layout(push_constant, std430) uniform Frame {
          ...
          uint  rowOffset;   // first image row this dispatch (band) covers
        } frame;
        ...
        // Max BVH traversal depth. A balanced median-split BVH of N leaves is ~log2(N)
        // deep, so this covers far more spheres than the host will ever generate; the
        // bound guards against a pathologically deep tree (a subtree is skipped rather
        // than overflowing the stack).
        const int BVH_STACK = 64;

        // Walk the BVH from the root device address, hopping pointers to find the
        // closest sphere hit. An explicit stack stands in for recursion.
        ...
        vec3 skyColor(vec3 dir) {
          ...
          return mix(cam.skyBottom, cam.skyTop, t);
        }
        ...
        vec3 rayColor(Ray ray, inout uint s) {
          ...
          for (uint bounce = 0u; bounce < MAX_BOUNCES; ++bounce) {
            ...
            if (!worldHit(ray, 0.001, 1e30, h)) {
              return attenuation * skyColor(ray.d);
            }

            int mat = int(h.material.x + 0.5);
            if (mat == 0) {
              // Lambertian: scatter about the normal.
          ...
          return vec3(0.0); // exceeded the bounce budget
        }

        void main() {
          // gl_GlobalInvocationID.y is relative to this band; rowOffset places it into
          // the full image. The host splits the render into horizontal bands so no single
          // compute submission runs long enough to trip the GPU's hang-recovery watchdog.
          uvec2 gid = uvec2(gl_GlobalInvocationID.x, gl_GlobalInvocationID.y + frame.rowOffset);
          ...
          for (uint i = 0u; i < SAMPLES; ++i) {
            float u = (float(gid.x) + rnd(s)) / float(frame.resolution.x);
            float v = (float(gid.y) + rnd(s)) / float(frame.resolution.y);
            // Image rows grow downward; flip v so the top row maps to the top of the
            // viewport.
            vec3 dir =
