...
with the per-object @transform@ from the object table (@gl_InstanceIndex@). Each
fragment writes @(objectId + 1, triangleId)@ into the @R32G32_UINT@ visibility buffer
(@0@ = background).
-}
...
vertCode =
  [vert|
    ...
    void main() {
      ...
      vObject = uint(gl_InstanceIndex) + 1u; // 0 = background
